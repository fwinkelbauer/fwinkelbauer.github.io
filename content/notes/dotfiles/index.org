...
This is my setup script for Linux Mint 22.2:

#+begin_src sh
sudo apt update
sudo apt install -y fonts-jetbrains-mono restic git emacs homebank keepassxc
...
[url "git@github.com:fwinkelbauer/"]
  pushInsteadOf = "https://github.com/fwinkelbauer/"
#+end_src
...
for remote in "${remotes[@]}"; do
    if [ -d "$remote" ]; then
        ...
        restic --repo "$remote" copy --from-repo "$local" -q
        restic --repo "$local" copy --from-repo "$remote" -q
    fi
done
#+end_src
