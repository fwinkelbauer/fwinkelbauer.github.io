...
#+begin_src sh
sudo apt update
sudo apt install emacs git homebank keepassxc libenchant-2-dev
...
(setq package-selected-packages '(embark-consult embark consult vertico orderless company magit color-theme-sanityinc-tomorrow jinx)
